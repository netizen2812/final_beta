...
const ScholarQuranManager: React.FC<ScholarQuranManagerProps> = ({ batchId, batchName, onClose }) => {
    ...
    useEffect(() => {
        const fetchAssignments = async () => {
            if (!batchId || activeTab !== 'track') return;
            try {
                const token = await getToken();
    ...
    return (
        <div className="flex flex-col gap-6 max-w-5xl mx-auto">
            ...
            {activeTab === 'assign' ? (
                ...
                <div className="bg-[#052e16] p-8 rounded-[3rem] border border-emerald-500/20 shadow-xl min-h-[500px]">
                    <h3 className="text-2xl font-bold text-white mb-6">Active Recital Queue</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {activeAssignments.length > 0 ? activeAssignments.map((a: any) => (
                            <div key={a._id} className="bg-black/40 border border-white/5 p-6 rounded-[2rem] flex items-center justify-between group hover:border-emerald-500/30 transition-all">
                                <div>
                                    <div className="font-bold text-white text-lg">{a.studentId?.name || 'Unknown Student'}</div>
                                ...
                                <button 
                                    ...
                                    className="bg-emerald-500 hover:bg-emerald-400 text-[#022c22] p-4 rounded-2xl font-black flex items-center gap-2 shadow-lg transition-all active:scale-95"
                                >
                                    <CheckCircle size={20} /> Mark Recited
                                </button>
                            </div>
                        )) : (
