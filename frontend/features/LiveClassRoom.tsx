    ...
    <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <h1 className="text-4xl font-serif font-bold text-[#052e16]">{t('live.liveQuranSessions')}</h1>
          <p className="text-slate-500 font-medium">{t('live.joinClasses')}</p>
        </div>
        ...
        {activeChild ? (
          <div className="bg-white px-5 py-3 rounded-2xl border border-emerald-50 shadow-sm flex items-center gap-4">
            ...
            <div><p className="text-[8px] font-black uppercase tracking-widest text-slate-400">{t('live.student')}</p><p className="text-sm font-bold text-[#052e16]">{activeChild.name}</p></div>
          </div>
        ) : (
          <div className="bg-amber-50 px-5 py-3 rounded-2xl border border-amber-100 flex items-center gap-3">
            <ShieldCheck size={20} className="text-amber-600" />
            <p className="text-xs font-bold text-amber-800">{t('live.selectChildSchedule')}</p>
          </div>
        )}
      </div>
  ...
  if (sessions.length === 0) {
    return (
      <div className="bg-white rounded-3xl p-10 border border-slate-100 text-center shadow-sm">
        ...
        <h3 className="font-bold text-slate-800 text-lg">{t('live.noActiveClasses')}</h3>
        <p className="text-slate-500 text-sm mt-1">{t('live.notEnrolled')}</p>
      </div>
    );
  }
        ...
        <div key={s._id} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-xs text-slate-400 font-medium">{t('live.batch')}: {s.title}</span>
            </div>
          ...
          >
            {s.status === 'active' ? t('live.joinClass') : t('live.openClass')}
          </button>
        </div>
      ))}
    </div>
