...
async function audit() {
    try {
        ...
        if (!syed) {
            console.log('Syed not found in Tarbiyah.children');
            const all = await childColl.find({}).toArray();
            console.log('Available children names:', all.map(a => a.name));
            return;
        }

        console.log('--- SYD DATA ---');
        console.log('ID:', syed._id);
        console.log('Streak (DB):', syed.child_progress?.[0]?.streak_days);
        console.log('Last Active:', syed.child_progress?.[0]?.last_active_date);
